...
═══ 참조 원문 (2026-08-12 확인) ═════════════════════════════════════════════
  ...
  ── 우리와 갈리는 곳 (실행 전 판단 필요) ──
    ...
    4. 검증이 **unseen PD gains** 다. 우리 계획(따로 뺀 구간 궤적)보다 강하다 —
  ...
  ── ★원문이 **하지 않은** 것 ──
    ...
    *"damping/friction separation assumed but not validated independently"* —
    즉 아래 JDAMP↔JFRIC 맞바꿈은 **원문도 다루지 않은 빈틈**이다. 이 도구가 그 자리다.
═════════════════════════════════════════════════════════════════════════════
...
def build_traj(mc, jm, cfg_all, T, rate, dual=None):
    ...
    dual=(ratio, f_slow): ★JDAMP↔JFRIC 분리를 겨냥한 변형 — **실측 결과 효과 없다.**

      ══ 2026-08-12 검증 (10s, 섭동 ±10%, 잡음 0.02°) ══
        기본 처프            JDAMP.foot ↔ JFRIC.foot  r = 0.935
        dual (0.35, 0.12Hz)                           r = **0.953**  ← 더 나빠졌다
        조건수 8.6 → 8.5 · JDAMP.foot 감도 0.0017 → 0.0011
      ★이유: 진폭 예산을 나눠 쓰므로(a_fast = amps × (1−ratio)) **고속 성분이 그만큼
        줄어든다.** 점성을 짚는 건 그 고속 구간이다 — 저속을 늘려 얻는 것보다
        고속을 잃는 손해가 크다. 예산을 안 나누려면 진폭·속도 한계를 넘어야 한다.
      ⇒ **JDAMP 는 각축 등속 스윕이 유일한 길이다**(몬테카를로 b 오차 1~6% vs
        처프 40~142%). 이 옵션은 그 결론의 근거로 남긴다.
      ══════════════════════════════════════════════
      점성 `b·q̇` 와 쿨롱 `τ_c·sign(q̇)` 는 **속도 크기가 한 종류면 구분되지 않는다**
      (한 속도에서는 둘 다 그냥 상수 토크로 보인다). 고정진폭 처프는 각 주파수에서
      속도 분포가 비슷해 이 맞바꿈이 남는다.
