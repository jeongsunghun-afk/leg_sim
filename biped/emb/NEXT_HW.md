...
### B. 코드 배선 (남은 작업)
- [x] **`spec.shm.rate_hz` 200 → 1000** — SHM 은 1kHz 인데 200Hz 로 폴링해 80% 를 버리고
      있었다. q̈ 를 수치미분으로 뽑으므로 직격. (체크리스트의 "≥1kHz" 를 그대로 따르지는
      않는다 — 요구 샘플레이트는 **가진 대역폭**에 비례하고 우리 f_end 는 4Hz 다)
- [x] **전류 저장 + `Kt·I` 교차검증** — `hwio` 가 `cur` 를 읽는데 `np.savez` 에서 빠져 있었다.
      체크리스트가 경고한 "보고토크가 kp/kd 재구성이면 순환" 을 가르는 **유일한 독립 근거**다.
      *(기존 hip 데이터 검증: MIT 재구성 R² 0.53 ⇒ 보고토크는 측정 기반. 축별로 매번 확인)*
- [x] **`pace_validate.py`** — 체크리스트 Phase 3·5(정합 검증 + 잔차 계통분석). 아래 F 참조
- [x] **`hwio.run_torque`** — τ_ff 가진 루프(kp=kd=0) + **위치 드리프트 워치독**
      (복원력이 없어 τ_ff 가 틀리면 계속 흘러간다 → 시작각 대비 8° 초과 시 즉시 limp)
- [ ] τ_ff 처프 시험 스크립트 (`act_identify_pace` 에 `--torque` 모드)
- [ ] **hold-out 궤적** 수집·검증 절차 (fit 미사용 데이터로 예측오차)
- [ ] 반복 측정 2회 + warm/cold 비교
...
- [ ] ③ calf (ch2·ch6) — **foot+thigh 기구고정**(홀드민감 −19.3 → −0.1%). 그러면 f_end 4Hz 가능
- [ ] ④ thigh (ch1·ch5) — **calf+foot 기구고정**(−56.2 → −0.0%). ★hip 고정은 소용없다
      (−56.1%) — 결합은 인접 원위 링크에서 온다
- [ ] 클램프 자세 기록 + 그 자세 `I_link` 재추출 · 지그 질량 기록(관성 오염 방지)
...
### F. ★Phase 3 정합 검증 — `pace_validate.py` (신규, 하드웨어 미접촉)
