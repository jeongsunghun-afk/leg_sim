...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **★★★근본원인 규명 완료(2026-07-30, 계측+6에이전트 감사)**: "왜 legged는 되는데 우리는 안 되나"를 객관적으로 규명. **①솔버 아님**: qpOASES(legged 정확 솔버)로 QP가 OPTIMAL, **EOM 잔차 1e-13, 접촉력 Σ=mg 정확**(t=0 LEGDBG 덤프). **②코드버그 아님**: 6에이전트 term-by-term 감사(EOM·base-FF·접촉력·마찰·측정매핑·qpOASES변환)=legged 원본과 byte-faithful, 부호/프레임/인덱스 오류 0. **③진짜 원인=legged의 순수 feedforward base task(aBaseFF)**. LEGDBG: 정적인데 aBaseFF=[0.53 전방,0,0.02|0.01,0.28피치,0.09], **전방 0.53 = MPC가 원한 순 18N 전방접촉력/37.9kg**(포트버그 아님, MPC 입력특성=aBaseFF가 m·a_com 충실반영). legged base task는 **PD 0의 순수FF**라, MPC가 매순간 고속 재계획해 base 루프를 닫아줄 때만 안정(base 뜨면 다음 MPC해가 힘↓). 내 **동기 브리지엔 그 고속 폐루프가 없어** FF가 전방·피치 가속을 되먹임 없이 주입→<1s 전복. **④결정적 확증**: aBaseFF 제거+순수PD(내 커스텀WBC·A WBIC 방식)로 바꾸니 **STANCE <1s붕괴 → 4~6s 유지**(t=2s tilt16°·z0.438). base PD(kp300)·관절kd=3(legged 정확복제)·base가중(300)·qpOASES 전부 단독으론 무효였고, **FF→PD 전환만이 효과**. 잔여 완만낙상(t6)=faithful포트 미세튜닝+연속MPC 부재(2차).
   - **★★★다중원인 확정 + 작동 수정(2026-07-30, 계측 전체궤적)**: 실패는 단일버그 아니라 **연쇄**(전부 측정확증): **(a)순수-FF base task**(legged PD=0)가 t=0에 base 가속 [0.53전방,0.28피치]=MPC 순18N전방력 명령, 되먹임없이 드리프트 개시. **(b)관절 kd=3 댐핑 누락**(legged는 HW단 τ_wbc+kd·(velDes−q̇), 내 브리지는 순수τ)→드리프트 무댐핑 적분. **(c)QP실패 누적**: base가 이탈상태로 가면 cold-start nWsr=20 QP가 못 풀어 status64(MAX_NWSR), qpFail 5(t0.1)→186(t0.5), 가드가 stale해 재사용→폭주→<1s 전복. (a)개시·(b)미제동·(c)추격실패의 합. **★결정적: QP실패는 원인이 아니라 드리프트의 결과** — 컨트롤러가 nominal 근처 유지시 QP는 계속 feasible(qpFail 동결). **★작동 수정(측정): base PD 되먹임(순수FF 대체) + kd=3 관절댐핑 → STANCE 안정(z=0.450 nominal·tilt~5°·qpFail 3동결, t0.9까지 확증)**. 반면 FF기본은 t0.1에 z0.497·qpFail 급증 발산. 즉 PD+댐핑이 로봇을 nominal에 잡아 QP를 feasible 영역에 유지=연쇄 차단.
   - **★★★★대반전=STANCE는 애초에 정상, "붕괴"는 테스트 오염(VX 기본 0.3)이었음(2026-07-30)**: 브리지 200행 `vx=getenv("VX")?...:0.3` → 내 stance 테스트가 VX 미지정으로 **0.3m/s 전진 목표**로 돌아, stance(4발고정) 게이트가 물리적으로 못 걷는 목표를 쫓다 앞으로 기울어 전복. 그 "20N 전방력·aBaseFF 0.53"이 전부 이것. custom WBC "solid"는 run_view(VX=0)로 테스트돼 **비교 자체가 불공정**이었음. **VX=0으로 legged 원형(순수FF·PD無·댐핑無) STANCE 완벽 안정(z=0.440·tilt0.2°·qpFail 7동결)**. ⇒ **WBC 버그 없었음, 앞선 (a)(b)(c) 다중원인 진단은 오염된 조건 위였음(정정)**.
   - **★표준 τ=τ_ff+τ_pd 반영(Bellicoso2016 ANYmal WBC, eth-50106-01.pdf, 식17-19)**: 모션추종 태스크=`r̈_des(FF)+K_P·posErr+K_D·velErr−J̇u`. base task를 FF+PD로(basePd_ 기본 ON), 저수준 `τ=τ_wbc+jkp(qDes−q)+jkd(q̇Des−q̇)`. **A의 WBIC도 동일 구성**(사용자 확인). **분리 실측**: ①순수FF=안정 ②FF+PD(JKD0)=안정(qpFail0!) ③+관절댐핑(JKD≠0)=붕괴. **관절 kd댐핑은 이상토크 MuJoCo서 WBC 정확토크와 충돌해 깨짐**(실로봇/Gazebo 액추에이터 모델용 층, 이상sim 불요). ⇒ **STANCE 표준해결=base task FF+PD(WBC출력만 적용)=식17-18=A WBIC 구조. qpFail=0 안정.**
   - **★★TROT 벽=4→2발 접촉전환 base-pop(2026-07-30, 미해결)**: VX=0.3(정상 명령) trot이 첫 스윙전환서 **base가 0.5~0.9로 솟구침**(주저앉음 아님=과지지) → QP infeasible(status37)/MAX_NWSR(64) → 폭주. **kp·스윙kp·nWsr(20→300)·연속스레드MPC 전부 무효**(전부 동일 실패, 정직). 기전=게이트가 발 들라 하면 WBC는 2발이 37.9kg 전부 지지하도록 힘계산하는데 드는 발이 순간 아직 접지(4발)→과지지→튕겨오름. legged가 Go1(12kg)서 되는 건 가벼움+힘 스무딩. **02_Leg(3배중량)+타이트토크서 전환이 marginal/infeasible**. =메모리 기록된 접촉전환 벽 재확인.
   - **★★종합 결론(정정·최종)**: **STANCE=표준 FF+PD로 완전 해결(qpFail0 안정), 솔버·코드버그·로봇/물리 문제 전무**(앞선 "실패" 진단은 VX오염 산물). **동적 TROT=4→2발 접촉전환 과지지 벽**(모델기반 동적의 문서화된 난제, kp/nWsr/스레딩 무효). 완전 model-based 동적보행은 접촉전환 스무딩(MPC 힘램프+실접촉감지) 필요=RL 트랙(타처)이 커버하는 영역. 자산=faithful포트·qpOASES·표준τ_ff+τ_pd·계측토글(LEGDBG/BASE_PD/BASE_NOFF/JKD/JKP/NWSR/NO_FALLBACK)·**작동 STANCE(WBC_LEGGED=1 VX=0, 기본 FF+PD)**. **정직한 다음 옵션**: (a)**브리지를 스레드 연속 MPC로 재구성**(legged 아키텍처, MPC 별도스레드+WBC가 최신정책 스왑) → FF가 제대로 작동, (b)legged_control 전체 스택 이식(ROS1/Gazebo), (c)메모리 확정방향=RL(DTC) 피벗. **정적 STANCE solid·MPC standalone 계획 정확은 불변**. 실험토글=FF_BASE·SOFT_ZF·DDP·SWING_JOINT·SETTLE 전부 기본 off면 정적 solid.
