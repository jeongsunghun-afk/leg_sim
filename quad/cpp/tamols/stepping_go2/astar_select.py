#!/usr/bin/env python3
"""stepping_go2/astar_select.py — multi-step stone-SELECTION planner (A* graph
search) for the TRAP stepping-stone fields (stones_trap_L{0..9}.csv).
...
Output: plan_astar_L{0..9}.csv with columns order,leg,stone_idx,cx,cy,top_z.
...
REACH_FACTOR = 1.35      # explored band vs nominal chain step (= column pitch)
...
def load_stones(lvl):
    rows = []
    with open(os.path.join(HERE, f"stones_trap_L{lvl}.csv")) as f:
...
def astar_side(stones, side, max_reach, dy_max, size_max, nx):
    """Min-cost stone route strip -> last column for one leg side.
    Returns list of stone dicts, or None."""
    usable = [s for s in stones if side_usable(s, side)]
    goal_cx = max(s["cx"] for s in stones if s["ix"] == nx - 1)
    ...
    def h(node):
        return W1 * max(0.0, goal_cx - node["cx"])
    ...
    while openq:
        ...
        closed.add(i)
        node = nodes[i]
        if node["ix"] == nx - 1:                      # corridor end reached
            route, j = [], i
            while j != -1:
...
def main():
    with open(os.path.join(HERE, "trap_meta.json")) as f:
    ...
    for lvl in range(NUM_LEVELS):
        ...
        pitch = by_grid[(1, 0)]["cx"] - by_grid[(0, 0)]["cx"]
        row_pitch = by_grid[(0, 1)]["cy"] - by_grid[(0, 0)]["cy"]
        size_max = max(s["size"] for s in stones)
        max_reach = REACH_FACTOR * pitch          # < 2*pitch at every level
        dy_max = row_pitch                        # one-row lateral shift

        routes = {}
        for side in ("R", "L"):
            r = astar_side(stones, side, max_reach, dy_max, size_max, nx)
        ...
        for side in ("R", "L"):
            ...
            assert routes[side][-1]["ix"] == nx - 1, f"L{lvl} {side} short"
        ...
        out = os.path.join(HERE, f"plan_astar_L{lvl}.csv")
