...
tau_imp    = np.zeros((n_frames, 5))   # Impedance 토크
tau_offset = np.zeros((n_frames, 5))   # tau_imp + tau_ff
...
for i in range(n_frames):
    ...
    J_a = compute_jacobian(theta_a[i])                      # 실제 자세 자코비안

    # WBIC (M,C 누락, G - J^T λ)
    tau_grav[i] = compute_gravity_torque(theta_t[i])
    tau_ff[i]   = tau_grav[i] - J.T @ lam_des[i]

    # Impedance Control: τ_imp = J^T [Kp_imp·Δx + Kd_imp·Δẋ]
    x_t_i  = x_t[i]                                        # 목표 발끝 위치 (Cartesian 직접)
    x_a_i  = forward_kinematics(theta_a[i])[-1]            # 실제 발끝 위치 (FK)
    dx_t_i = x_t_dot[i]                                    # 목표 발끝 속도 (Cartesian 직접)
    dx_a_i = J_a @ dtheta_a[i]                             # 실제 발끝 속도 (FK)
    ...
    # CSP: τ_cmd = kp·Δq + kd·Δq̇ + τ_offset
