...
★왜 필요한가 (2026-08-12)
  ...
  (리포트 0.454/0.584 Nm 은 마찰이 아니라 '어긋난 bias 에서 기동까지의 거리' 였다).
...
MARGIN = 0.97      # 관절한계의 이 비율까지만 — 한계구속 반력이 섞이면 값이 50Nm 로 튄다.
# ★0.90 → 0.97 (2026-08-12). 0.90 은 **실기 자세를 못 담았다**: 오른다리를 놓으면
#   HR_hip 이 바깥으로 벌어져 모델각 +13.79° 에 서는데, 표는 ±13.41° 까지만이라
#   np.interp 가 끝값(+3.812 Nm)으로 **고정**했다. 그 어긋난 FF 로 홈복귀가 스톨했다.
#   ⚠0.97 이 한계구속을 건드리지 않는지는 아래 생성 로그의 τ 범위로 확인할 것 —
#     구속이 섞이면 수십 Nm 로 튀므로 눈에 바로 띈다.
