...
BRAKE_X = float(os.environ.get("BRAKE_X", "99"))       # ★위치-스케줄 제동: base_x>BRAKE_X면 제동ON(post-crossing settle). 갭 넘는동안 BRAKE(0)로 날고 넘은뒤 제동해 착지
...
def _mpc_run():
    ...
    def build_ref(phase, base_x0, base_y0, fworld, vx_meas):
        ...
        for k in range(Nh + 1):                      # nodes differ little → few IK iters each
            ...
            ref[k, :nq] = q_mj
            _bk = 1.0 if base_x0 > BRAKE_X else BRAKE      # 갭 넘은뒤(base_x>BRAKE_X) 제동ON=착지
            ref[k, 0] = base_x0 + (_bk * VX + (1.0 - _bk) * vx_pred) * k * DT  # 위치-스케줄 제동
