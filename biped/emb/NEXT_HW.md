...
## 10. ★매단 상태 관성 측정 → hip 중력보상 (`tau_ff`) — 다음 작업 (2026-08-10 계획)

목적이 둘로 나뉘고 **순서가 중요하다**: ①에서 잰 값이 ②의 입력이다.
그리고 ②는 **토크 경로의 첫 실기 시험**이다 — 지금까지 검증한 건 jog/home/hold(위치 명령)뿐이고
`write_torque`(kp=kd=0 순수 토크)는 실기에서 한 번도 돌지 않았다.

### 10-a. ① 매단 상태에서 `I_total` 측정 (PACE)

도구: `pace/tests/act_identify_pace.py` (처프 가진 → 최소자승 동정)

**전제 — 자세를 고정해야 한다.** hip `I_link` 는 무릎을 −40° 접으면 **+15%** 변한다(§11-a).
⇒ 측정 자세를 **기준자세 `[0,35,−60,60,…]`** 로 고정하고, 그 자세의 `I_link` 를
   `pace/extract_ilink.py` 로 **다시 뽑아** 비교 기준으로 삼을 것. 홈(q=0) 값을 쓰면 안 된다.

측정으로 얻는 것:
| 축 | 무엇을 확인하나 |
|---|---|
| hip·thigh | `I_total − ROTOR_I·N²` 가 MJCF `I_link` 와 맞는가 ⇒ **MJCF 검증** |
| calf·foot | 링크비중이 26%/3.7% 뿐이라 `I_total` 이 사실상 `ROTOR_I·N²` ⇒ **ROTOR_I 재확인** |
| 전 축 | `JDAMP`/`JFRIC` — 지금 thigh/calf/foot 값은 **hip 외삽**이다(§9 미확정) |

⚠**중력항 주의**: 매단 상태에서 hip 을 가진하면 다리 전체가 중력 토크를 만든다.
  `act_identify_pace.py:23-26` 은 `cos(q)` 를 회귀에 **넣지 않는다** — 가진 진폭이 작으면
  상수열과 공선이 되어 조건수가 16348 까지 뛰고 중력·바이어스가 ±13 Nm 로 서로 상쇄되는
  무의미한 값이 나오기 때문이다. 그래서 얻어지는 `A` 는 절대 중력이 아니라
  **"가진 범위 내 중력 변화폭"** 이다. 다리가 붙은 지금은 그 변화폭이 커졌으므로
  **가진 진폭을 작게** 유지해 선형 근사가 유효한 구간에 머물러야 한다.

⚠안전: PACE 전에 §5 안전한계 재산정을 먼저 볼 것. 토크 트립 15 Nm(채널, 50 ms 연속)에
  걸리면 래치 limp 가 되고 **off 명령으로만 풀린다**.

### 10-b. ② hip 중력보상 — `tau_ff` 만으로 다리를 들고 있기

**보고 싶은 것**: `kp=kd=0` 으로 두고 `tau_ff` 만 넣었을 때 다리가 처지지 않고 떠 있는가.
성공하면 모델(질량·CoM·자코비안)과 토크 경로(`gear_k` 나눗셈 + 커플링 전치)가 **동시에** 맞다는 뜻이다.

중력토크 산출 — MuJoCo 가 그대로 준다:
```python
d.qvel[:] = 0; d.qacc[:] = 0
mujoco.mj_forward(m, d)
tau_g = d.qfrc_bias[6:]        # freejoint 6 dof 를 건너뛴 관절 중력항 [Nm]
```
매단 상태이므로 **base 를 고정**해야 한다(free joint 속도를 0 으로 유지하거나 qfrc_bias 의
관절부만 사용). 실기 자세를 `q_leg_deg` 로 받아 `qpos[7:]` 에 넣고 계산한다.

전달 경로:
```python
hw.write_torque(q_ctrl_rad, dq_ctrl_rad, tau_ff_nm, kp_leg=0.0, kd_leg=0.0)
```
`tau_ctrl_to_ch` 가 **k 로 나누고 커플링을 전치로** 넣는다(§RL_INTERFACE §1). hip 은 k=1 ·
커플링 없음이라 가장 단순한 축이다 — **첫 시험을 hip 으로 하는 것이 옳다.**

⚠**순수 토크는 위험하다** — 오차가 있으면 적분되어 다리가 서서히 떨어지거나 튄다.
  ⇒ 단계적으로:
  1. `tau_ff = 0`, `kd` 만 소량(예: 1.0) — 다리가 천천히 처지는지 확인(감쇠만 동작)
  2. `tau_ff = α·tau_g`, α 를 0 → 1 로 **손으로 올리며** 처짐이 줄어드는지 관찰
  3. α=1 에서 뜨는가. 남는 처짐이 있으면 그게 **모델 오차 + 정지마찰**(hip 정지마찰
     실측 0.63~0.71 Nm)이다
  ⚠hip 중력토크는 홈자세 기준 **4.96 Nm** — 트립 15 Nm 대비 여유가 있으나 α 를 1 넘기지 말 것.

**성공 판정**: α=1 에서 남는 처짐이 정지마찰로 설명되는 크기(≈0.7 Nm/kp 상당) 이내.

### 10-c. 이 두 작업이 풀어 주는 미확정 항목
`RL_INTERFACE.md` §9 의 다음이 해소된다:
`JDAMP`(hip 외삽) · `JFRIC`(hip 외삽) · `ROTOR_I`(타축 가정) · torso 관성(간접) ·
그리고 **토크 경로 자체의 실기 검증**(현재 §7 "모델기반 경로 미검증").

### 10-d. 선행조건
- [ ] `RobotEmbedded` 재빌드 후 기동 램프가 **무동작**인지 확인(§10 기동 램프 제자리유지 패치)
- [ ] 기준자세 재현 확인 — 다시 놓았을 때 `[0,35,−60,60,…]` 로 읽히는가
- [ ] hip·thigh 물리 하드스톱 확인(jog 를 관절한계 전 범위로 열어 둔 상태)
