...
def g_friction(x0, pfx="f"):
    ...
    # 기본바닥 μ=1.0. 만나는 순서=1.0(바닥)→0.8→0.4→0.1(점점 미끄러움).
    wid=1.6; llen=0.6; s=""; x=x0+llen
    lanes=[("f2","0.80 0.02 0.001",   "0.60 0.62 0.66 1"),  # μ0.8(보통, 회색)
           ("f1","0.40 0.01 0.0005",  "0.62 0.72 0.85 1"),  # μ0.4(미끌, 연파랑)
           ("f0","0.10 0.005 0.0001", "0.45 0.80 0.98 1")]  # μ0.1(빙판, 파랑)
    for nm,mu,rgba in lanes:
        s+=box(nm, llen, wid/2, 0.012, x, 0, 0.012, rgba, fric=mu, prio=1); x+=2*llen
    return s, x
