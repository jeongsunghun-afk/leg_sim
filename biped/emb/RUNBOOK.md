...
`./run_hw.sh` 는 3D 자세 뷰어·GUI 를 배경으로 띄운 뒤 **그 터미널에서 값 모니터로 이어진다**
(2026-08-13 추가). 3D 뷰어는 자세만 보여주지 명령을 얼마나 못 따라오는지는 안 보인다 —
stand 처럼 정적인 동작에선 그 편차가 판단의 전부다.

```bash
./run_hw.sh                    # 뷰어 + GUI 를 띄우고, 이 터미널이 값 모니터가 된다
                               #   Ctrl-C = 모니터만 종료(뷰어·GUI 는 계속 돈다)
MON=0 ./run_hw.sh              # 종전처럼 프롬프트로 바로 복귀
MON_ARGS="--ch --hz 20" ./run_hw.sh          # 채널각도 함께 · 20Hz
QUAD_STATE=/tmp/biped_state.json python3 monitor_state.py    # 따로 띄우기(SSH 가능)
```

⚠**별도 창으로 띄우지 않는다.** 처음엔 `x-terminal-emulator` 로 창을 띄웠는데 안 떴다 —
이 기기의 그것은 `gnome-terminal.wrapper` 라 폐기된 `-e` 분기로 갔고, gnome-terminal
런처는 서버에 붙은 채 반환하지 않으며, 에러를 `/dev/null` 로 버려 조용히 실패했다.
터미널 에뮬레이터마다 인자 규약이 달라 그 분기는 계속 깨진다 ⇒ 의존을 없앴다.
