...
def measure_inertia_torque(hw, spec, joint, plotdir, log=print) -> tuple[str, dict]:
    ...
    #   ⚠⚠**방향과 혼동될 수 있다** (2026-08-14 실기 후 확인). `--solo` 에서는 아래 링크가
    #     자유라 유효관성이 **방향에 따라 다르고**, − 방향이 + 보다 t_ref 가 체계적으로
    #     길다. 그러면 "I 가 t_ref 에 따라 는다" 는 관계가 **방향 차이의 그림자**일 수 있다.
    #       HL_thigh: 합쳐서 외삽 0.2003(+6%) · **+방향만 0.1584(−16%) · −방향만 0.2102(+11%)**
    #       ⇒ 두 방향이 28% 갈린다. 합친 +6% 는 평균이 우연히 CAD 근처에 온 것이다.
    #     ⇒ **방향 편차가 게이트(15%)를 넘으면 외삽값도 믿지 말 것.** 아래에서 같이 찍는다.
    ...
    if len(_pts) >= 4 and tau_break:
        ...
        log(f"  [{name}] ★t_ref→0 외삽(2차) — {len(_A)}점 · t_ref "
            ...
            f"**I_joint {_I0:.5f}**")
        # ★방향 편차가 크면 외삽도 못 믿는다 — 위 주석의 혼동이 그때 일어난다
        _ds = res.get("dir_spread")
        if _ds is not None and _ds > 0.15:
            log(f"           ⚠⚠방향간 I 편차가 {_ds*100:.0f}% 다 — **이 외삽값도 믿지 말 것.**"
                f" t_ref 의존이 아니라 방향 차이를 맞춘 것일 수 있다(주석 참조)")
        if _span < 2.0:
