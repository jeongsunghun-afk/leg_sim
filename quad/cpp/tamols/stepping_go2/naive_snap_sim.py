#!/usr/bin/env python3
"""stepping_go2/naive_snap_sim.py — baseline: the RL env's NAIVE nearest-stone
snap marched over the TRAP stone fields (stones_trap_L{0..9}.csv).
...
a stone; strip->strip excluded, strip->first-stone included):
  ...
  (c) % of on-stone footfalls landing on designated TRAP stones (trap_meta.json).

Output: naive_trap_L{0..9}.csv (per-footfall) + per-level table on stdout.
Usage: python3 naive_snap_sim.py [--adv 0.216]   (default: calibrated 0.24)
"""
import argparse
import csv
import json
...
def load_meta():
    ...
    tm = json.load(open(os.path.join(D, "trap_meta.json")))
    return pm, tm
...
def load_stones(lvl):
    rows = []
    with open(os.path.join(D, f"stones_trap_L{lvl}.csv")) as f:
        for r in csv.DictReader(f):
...
def run_level(lvl, tm_lvl, adv, x_snap, anchors, write_csv=True, rng=None,
    ...
    if write_csv:
        ...
        with open(os.path.join(D, f"naive_trap_L{lvl}.csv"), "w", newline="") as f:
