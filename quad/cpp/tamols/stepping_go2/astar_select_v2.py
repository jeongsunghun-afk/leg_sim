...
(stones_trapv2_L{0..9}.csv). v2 adaptation of astar_select.py: the fields have
...
Output: plan_astarv2_L{0..9}.csv with columns order,leg,stone_idx,cx,cy,top_z.
...
MAX_REACH = 0.53         # v2 fixed reach band (golden chain dx <= 0.45 < this)
DY_MAX = 0.55            # v2 fixed lateral band (golden chain |dy| <= 0.50)
...
def load_stones(lvl):
    rows = []
    with open(os.path.join(HERE, f"stones_trapv2_L{lvl}.csv")) as f:
...
def edge_feasible(a, b, max_reach, dy_max):
    ...
    if dz > DZ_MAX + 1e-12:
        return None
    if a["idx"] != -1 and math.hypot(dx, dy) > 0.53 + 1e-9:
        return None          # calibrated center-jump band upper (plans <=0.528)
...
def main():
    with open(os.path.join(HERE, "trapv2_meta.json")) as f:
    ...
    for lvl in range(NUM_LEVELS):
        ...
        out = os.path.join(HERE, f"plan_astarv2_L{lvl}.csv")
