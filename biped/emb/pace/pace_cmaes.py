...
═══ 2026-08-14 미해결 — **구조 증거와 적합 결과가 어긋난다** ═══════════════

foot PD 오차를 raw각(q_foot+q_calf)으로 재느냐 관절각으로 재느냐가 갈리지 않았다.

★구조 증거는 **네 갈래 전부 raw** 를 가리킨다
  ① 관성 비   무릎 자유/고정 = **1.06** (예측: tendon 0.99 · 커플링없음 0.64)
               → 전동(tendon)이 실재한다. 힘 쪽은 확정이다.
  ② 엔코더    무릎 143° 돌려도 foot 채널각 불변(기울기 < 2e-4, 좌우 모두)
               → 엔코더는 모터축에 있다 = raw각을 읽는다
  ③ MJCF      `*_foot_rotor` tendon 길이 = q_calf+q_foot = **엔코더 좌표와 같다**
  ④ 명령경로  `joint_map.q_joint_to_ch` 가 raw[d] = qj[d] + coef·qj[src] 로 되먹인다
               → 드라이버가 받는 목표도 raw 다

★그런데 적합은 **네 번 다 관절각**을 고른다 (교차분할·같은 상자·같은 고정)
      조건                     A(raw) 적합/holdout   B(joint) 적합/holdout
      tail 분할                 0.3568 / 0.7711       0.3782 / 0.5000
      교차분할                  0.5067 / 0.5573       0.3963 / 0.3888
      교차분할 +JDAMP.foot 고정  0.4641 / 0.4980       0.4142 / 0.4241
  delay 도 같은 방향이다 — A 는 하한 벽(6.82~6.85ms), B 는 상자 안(8.48~9.21ms).
  실측 8.39±0.79 를 지키는 쪽이 B 다.

★내 최선의 해석(확정 아님): raw 법칙은 **calf 의 추종오차를 foot 토크에 실어 준다.**
  그래서 calf 모델오차가 foot 으로 증폭돼 들어간다. 실제로 calf 는 지금 잘 안 정해진다
  — JFRIC.calf 가 법칙에 따라 하한(0.701)과 상자안(0.817)을 오가고, JDAMP.calf 도
  0.058↔0.269 로 뛴다. 관절 법칙은 둘을 떼어 놓아 그 오염을 막는다.
  ⇒ **적합이 좋은 쪽이 물리적으로 맞는 쪽이라는 보장이 없다.** 구조를 버리면 안 된다.

★가르는 방법 — **unseen PD gains** (원문 PACE 의 주 검증, `--validate` 로 이미 지원)
  다른 kp/kd 로 수집한 두 번째 데이터셋이 필요하다. 게인이 바뀌면 "잘못된 구조로
  게인에 얹혀 맞춘" 쪽이 무너진다. 궤적 hold-out 보다 훨씬 강하다.
  ⚠그때까지는 **배포·sim2real 에는 구조가 맞는 쪽(raw, 기본값)을 쓴다.** 적합수치가
    좋다고 틀린 구조를 배포하면 궤적이 바뀌는 순간 깨진다.
