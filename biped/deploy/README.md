...
| **전진 + 지연**(30s) | — | 무보상 구동≤5ms·센서≤10ms=**0** (`SENSE/ACT_LAT_MS`) |
| **전진 + 지연 + 지연보상**(30s) | — | 모델롤아웃 보상: 구동≤~12ms·센서≤~15ms=**0** (~2배↑, `LAT_COMP_MS`) |

**배포 스펙**: 노이즈 강건(4배 OK). 지연은 점발 marginal이라 민감하나 **지연보상으로 완화**:
- 무보상: 구동지연 ≤5ms 필요(빠듯).
- **모델롤아웃 보상**(in-flight 토크로 동역학 전진 적분=Smith predictor): **구동 ≤~12ms·센서 ≤~15ms**로 완화 → **현실 HW(EtherCAT~1ms+모터~5–10ms) 달성 가능권**. 합산 총지연 ~10–15ms가 상한.
- 보상 비용: 제어 스텝마다 LCOMP회 mj_step(C++서 저렴). `LAT_COMP_MS`(kinematic=`LAT_COMP_KIN=1`).
- 상한 넘는 지연/외란 강건성은 RL 몫(잔여).
