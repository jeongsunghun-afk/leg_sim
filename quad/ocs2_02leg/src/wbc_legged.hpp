...
class WbcLegged {
 public:
  ...
  vector_t update(const vector_t& stateDesired, const vector_t& inputDesired, const vector_t& rbdMeasured, size_t mode,
    ...
    // eiquadprog: min .5xᵀGx+gᵀx s.t. CE x+ce0=0, CI x+ci0≥0.
    ...
    // ── soft 비용: swing*wSw + baseAccel*wBase(FF) + force*wF ──
    matrix_t G = matrix_t::Zero(nx_, nx_); vector_t g = vector_t::Zero(nx_);
    auto addCost = [&](const std::pair<matrix_t, vector_t>& t, double w) {
      if (!t.first.rows()) return;
      G.noalias() += w * t.first.transpose() * t.first;
      g.noalias() -= w * t.first.transpose() * t.second;
    };
    addCost(swingTask(), wSwing_);
    addCost(baseAccelTask(), wBase_);
    addCost(contactForceTask(inputDesired), wForce_);
    // eiquadprog는 G positive-definite 요구(qpOASES는 semi-def 허용). τ·joint-q̈ 블록이 비어
    // near-singular → 소량 정규화(최소노름 해 선택, 물리 왜곡 없음).
    G.diagonal().array() += reg_;
