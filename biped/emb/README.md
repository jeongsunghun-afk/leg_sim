...
⚠쓰기 마지막에 채널각을 **±180 으로 포화**시킨다. **★2026-09-02 정정** — 종전 근거
("Emb 가 래핑해서 181° 가 −179° 로 뒤집힌다")는 **틀렸다.** 래핑 코드는 삭제됐고,
살아 있는 클램프(`halGait.cpp:716-717`)는 벤더 자체 경로 전용이라 우리 명령은
`:782` 의 MotCmd memcpy 가 통째로 덮어써서 **지나가지도 않는다.**
⇒ **Emb 는 우리 명령을 검사하지 않는다** — 위치·게인·토크가 그대로 통과한다.
  이 포화가 **유일한 가드**이고, 넘으면 점프가 아니라 **조용히 잘린다**(경고 없는 오차).
