...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **★★종합 결론(정정·확정)**: legged WBC 이식 실패는 **솔버도 코드버그도 로봇/물리 전이도 아니고, legged의 순수-FF base task가 '연속 고속 MPC 폐루프'와 co-design된 것**이라 그 루프 없이 WBC만 떼오면 발산하는 것. **우리는 이미 동일 로봇서 동작하는 WBC 보유**(custom WBC·A의 WBIC=PD 되먹임). 즉 "다른 데선 되는데 우리는 안 됨"의 답=legged는 WBC가 자기 연속-MPC 루프 안에서 돌아 되고, 우리는 그 WBC를 다른 루프(동기 브리지)에 붙여 안 됨. **완전 이식 경로=(a)브리지를 연속 스레드MPC로 재구성 or (b)base task를 PD로(legged 원형 이탈=사실상 내 custom WBC로 수렴)**. 동적보행 산출물은 RL 트랙(타처)이 커버. 자산=faithful포트·qpOASES·계측토글(LEGDBG/BASE_PD/BASE_NOFF/JKD). **정직한 다음 옵션**: (a)**브리지를 스레드 연속 MPC로 재구성**(legged 아키텍처, MPC 별도스레드+WBC가 최신정책 스왑) → FF가 제대로 작동, (b)legged_control 전체 스택 이식(ROS1/Gazebo), (c)메모리 확정방향=RL(DTC) 피벗. **정적 STANCE solid·MPC standalone 계획 정확은 불변**. 실험토글=FF_BASE·SOFT_ZF·DDP·SWING_JOINT·SETTLE 전부 기본 off면 정적 solid.
