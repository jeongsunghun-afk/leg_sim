...
def ik_objective(theta, target, link_lengths):
    foot = forward_kinematics(theta, link_lengths)[-1]
    return np.linalg.norm(np.array(foot) - np.array(target))
...
target_position = list(forward_kinematics(theta, link_lengths)[-1])
history = []
playback_mode = False
frame_idx = 0
...
def update_visual():
    ...
    x_axis_line.set_data([foot[0], foot[0] + dx], [foot[1], foot[1] + dy])
    x_ortho = -dy
    y_ortho = dx
    y_axis_line.set_data([foot[0], foot[0] + x_ortho], [foot[1], foot[1] + y_ortho])
...
plt.title("4DOF Leg Simulation with Reset and Playback Toggle")
plt.show()
