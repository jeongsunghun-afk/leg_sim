...
#
#   ★JOBS 는 **코어수-2 가 아니다.** 실측(WSL2 · 22논리코어 · 15GB, 2026-08-20):
#         JOBS= 8   22런 34s   실패 0    ← 최적
#         JOBS=11   22런 43s   실패 0
#         JOBS=14   22런 46s   실패 0
#         JOBS=20   20런 80s   **실패 9/20**   sys 4m46s (스레드 과다구독)
#     과다구독은 느려지는 데서 그치지 않고 **런이 죽어 sentinel 이 된다**(7c608c4 의 `err` 열).
#     ⇒ 8 로 시작하고, 올릴 거면 `err` 가 0 인지 보고 올릴 것. 최종 채택점만 Pi 에서 재확인.
