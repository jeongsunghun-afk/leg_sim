...
### A. 선행 (측정 전에 반드시)
- [ ] **지그로 영점 재캘리브레이션** — `diag/calib_zero.py --apply` (정지·재현성 게이트 통과 확인)
- [ ] 영점 후 `prep_inertia.py` 재실행 — 새 자세 기준 `I_link`·안전진폭 다시 뽑기
- [ ] `RobotEmbedded` 기동 램프가 **무동작**인지 확인 (제자리유지 패치, 재빌드 완료됨)
- [ ] hip·thigh **물리 하드스톱** 확인 — jog 를 관절한계 전 범위로 열어 둔 상태다
- [ ] 크레인·거치대·배선 간섭 육안 확인 (MJCF 에는 없는 것들)

### B. 코드 배선 (남은 작업)
- [ ] `actuator_test.py --home-first` — `hwio.goto_all` 호출해 홈 정렬 후 측정 시작
- [ ] `spec.yaml` 갱신 — `I_link`(측정 자세 기준) · `amplitude_deg` 6.0 → 안전값
- [ ] **홀드축에 `tau_ff` 중력보상 추가** (`kp` 는 그대로 유지 — 둘은 직교, §10-c2)
- [ ] calf·thigh 용 **저주파 처프 프리셋** (`f_end` 2.4Hz · 진폭 5~9°) 추가
- [ ] `kp_max 60` 이 홀드 강성의 상한임을 리포트에 명시 — 올릴지는 별도 판단
- [ ] PACE 결과에 **k² 복원**과 `check_hold` 결과를 리포트에 함께 출력

### C. 관성 측정 (순서 고정)
- [ ] **① foot** (ch3·ch7) — 측정 체계 자체 검증. `ROTOR_I` 확정
- [ ] ② hip (ch0·ch4) — 홈 정렬 필수. **`I_link` = MJCF 검증**
- [ ] ③ calf (ch2·ch6) — **f_end 2.4Hz** 프리셋. `check_hold` 확인 필수(홀드민감 −19%)
- [ ] ④ thigh (ch1·ch5) — **f_end 2.4Hz**. 홀드민감 −56% ⇒ foot/hip 에서 확정한
      `ROTOR_I`·`I_link` 를 **기지수로 고정**하고 회귀할 것. 자유 회귀는 신뢰 불가
- [ ] 축별 `JDAMP`/`JFRIC` 도 같이 확정 (현재 전부 hip 외삽)

### D. 중력보상
- [ ] `tau_g` 계산 도구 (`qfrc_bias`, base 고정)
- [ ] hip 단축 `tau_ff` 시험 — α 0→1 단계적
- [ ] 남는 처짐이 정지마찰(0.63~0.71 Nm)로 설명되는가
- [ ] 성공 시 → **토크 경로 실기 검증 완료** (RL_INTERFACE §7 미검증 항목 해소)

### E. 문서 반영
- [ ] `RL_INTERFACE.md` §9 미확정 표 갱신 (`ROTOR_I`·`JDAMP`·`JFRIC` 해소분)
- [ ] `RESULTS.md` 에 매단 상태 측정 결과 절 추가
- [ ] `spec.yaml` 의 `I_link` 를 **자세와 함께** 기록 (자세 없는 관성값은 의미가 없다)
