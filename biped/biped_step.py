...
#   ★2026-08-12 재스윕 0.38 → **0.34** (새 CAD 몸통). 위 "되돌리지 말 것"은 **구 CAD 기준**이다.
#     0.38 은 구 몸통(3.0kg placeholder, CoM x=0)에서 나온 값이고, 실측 몸통(2.8kg,
#     CoM x=−0.0727)·고관절 26cm 이동 후에는 **후진이 6.5초에 깨진다**.
#     C++ 실측 스윕(Q_HOME 재산출 후, biped_from_quad 점발 15s):
#         T 0.38  vx −0.10 = 6.54s 낙상 · 0.20 = 2.13s 낙상   (전진 0~0.15 만 생존)
#         T 0.42  vx −0.10 = 5.45s · 0.15 = 2.56s 낙상
#         T 0.34  **−0.15/−0.05/0/0.05/0.10/0.15/0.20 전부 15s 무낙상, tilt 0.2~1.9°**
#     ⚠순서가 중요하다 — Q_HOME 을 먼저 고쳐야 이 값이 산다. 구 자세에서는 T 를 어떻게
#       잡아도 전방 폭주가 남는다(0.30 하나가 폭 ±0.005 짜리 칼날 위에서만 살았다).
T_STEP  = 0.34          # ★새 CAD 재스윕값(2026-08-12). 구 CAD 기준값은 0.38 이었다
