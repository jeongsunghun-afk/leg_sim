...
struct BipedEstimator {
  ...
  // ★★2026-08-19 `alpha` 가 **제자리 드리프트의 진범**이다 (EST_ALPHA 로 조정).
  //   `v = (1−α)·v + α·v_meas` 는 저역통과라 **지연**을 만든다. 제자리 stepping 은
  //   스탠스가 바뀔 때마다 속도가 급변하는데, 그 지연이 **매 스텝 같은 방향으로 누적**된다.
  //   배포조건(지연 8.4ms · 실측 노이즈) · 5시드 · 30초 · 생존런 추정오차 |est−GT|:
  //       α 0.2 → 0.235 m   α **0.4(기본)** → 0.182 m   α 0.7 → 0.104 m   α **1.0 → 0.034 m**
  //     단조롭다. 필터를 **완전히 끄면 오차가 5.4배 줄고** 낙상도 0/5 다.
  //   ⚠역설적이지만 α 는 원래 **노이즈를 잡으려고** 있던 것이다. 그런데 실측 노이즈를
  //     넣은 조건에서도 끄는 쪽이 이긴다 — leg-odom 속도는 이미 두 발 평균이라
  //     충분히 매끄럽고, 필터로 얻는 것보다 **지연으로 잃는 것이 크다**.
  //   ⚠**아직 기본값을 안 바꿨다.** 검증이 vx=0 · 5시드뿐이다. 보행 속도대(0.10/0.20)와
  //     시드 30+ 로 확인한 뒤 바꿀 것. 후보 축은 sweep_paired.sh 의 ALPHAS 로 옮겨뒀다.
  //   ⚠미끄러짐·스윙발 오염은 **아니다** — 바닥마찰 1.6→4.0 은 오히려 악화했고,
  //     착지 과도 제거(dwell)는 추정오차를 2.6배로 키웠다(아래 dwell 절).
