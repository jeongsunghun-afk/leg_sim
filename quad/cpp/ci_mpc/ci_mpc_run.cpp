...
int main(){
  ...
  double TGT_BZ=envd("TGT_BZ",-1.0), LIFT_H=envd("LIFT_H",0.10), PITCH=envd("PITCH",0.0);   // PITCH=base nose-up(rad)
  int LIFT=envi("LIFT",-1), LIFT_FRONT=envi("LIFT_FRONT",0);   // LIFT_FRONT=1: FL·FR 둘 다 들기(두발서기)
  ...
  if(LIFT_FRONT){ pnom[0][2]=LIFT_H; pnom[1][2]=LIFT_H; }   // 앞 두발 들기(두발서기)
  double CX=envd("COM_X",0.0), CY=envd("COM_Y",0.0);   // ★base CoM을 (CX,CY)로 이동(축소 지지면 위로)
