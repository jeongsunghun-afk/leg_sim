...
- 아티팩트 재배포: `docs/*.html` 편집 → `Artifact` 도구로 각 URL에 갱신. **★재배포 시 반드시 아래 고정 URL을 `url=`로 지정**(미지정 시 새 URL 발급되어 파편화·기존 링크 stale).
  - `pipeline_bc.html`(구 pipeline_bcd1에서 D1 분리) → https://claude.ai/code/artifact/0aedd22e-f124-43a4-908c-11fda4823d49 (구 bcd1 URL 재사용)
  - `pipeline_d1.html`(D1 OCS2 포팅, pipeline_tamols 양식) → (URL 미기록 — 다음 재배포 시 발급되는 URL을 여기 기록할 것)
  - `pipeline_tamols.html` → (URL 미기록 — 다음 재배포 시 발급)
