...
## 추기 2026-09-04 심야 — CoM 서기 역설 최종 해소 (워크플로 3관점+반증)

**역설(공중 뒤·서기 앞)은 모순이 아니었다**: 질량 68%가 다리 → 매달림은 다리가 뒤로
늘어져 CoM frac<0, 서기는 다리 접혀 68%가 앞으로 → CoM frac 0.65. 같은 로봇 다른 자세.

**내 오류 2개 확정 정정**:
 · "시뮬 발목 0.2Nm→60배" = **가짜**. 0.2는 base-fixed qfrc_bias 아티팩트.
   **정직한 접지 발목토크 = 7.6 Nm/발** (CoM 발목축(뒤꿈치) 앞 9.5cm × W159N ÷2).
   사용자 지적("CoM 발목 앞이니 토크 필요") 정확.
 · "12Nm 순전히 인공값" = 절반. 정직 7.6, toe 가 CoP 를 발끝으로 밀어 11.6 과주입.
   12/7.6=1.6배 ⊂ 전달비결손 2배.

**toe=목발 확정**: mid(foot명령5.8×1.3÷2배전달≈3.8) < 정직7.6 → 주저앉음.
 toe(11.6→÷2≈7.5) ≈ 정직7.6 → 유일통과. 발끝레버로 명령 2배뻥튀겨 결손 상쇄.
 근본해 = ①drive 전달비 교정(=전류제한) ②주입점 frac0.7 ③전후피드백(IMU).

**payload x=0 유지** — 서기 무관(Jᵀ·F 기하). frac0.5 로는 x=−0.154(비물리).
 2-저울로 서기 CoM frac 직접측정 후에만 조정. 앞서 x 를 앞뒤로 옮긴 것 철회.

**로버스트 서기 = IMU 하드게이트**: toe 개루프는 발끝 칼날평형(수초용), 전후피드백 0.
 IMU or 2-로드셀 pitch 관측기 선결.

**다음 측정 2개**(각도 눈측정 대체):
 · 2-저울(뒤꿈치·발끝선): CoP_x=ΣFx/ΣF = frac 직접. 0.65면 모델옳음·toe과주입 / 1.0이면 전방질량
 · FF 계단 프로브(발오차→0): 전달비결손 배수 직접
