...
struct MockHw : HwIface {
  ...
  int  f_stuck_ch=-1;                 // ★FAULT_STUCK_CH=n → 그 채널만 명령 무시(토크 불응답 재현).
                                      //   health=ok·에코 정상인 채 안 움직이는 08-28 실기 현상.
  MockHw(int nch, double dt_, double max_dps=60.0)
    ...
    f_stuck_ch = (int)ev("FAULT_STUCK_CH", -1);
  }
  bool init(int) override {
    ...
    if(f_stuck_ch>=0) std::printf("[MockHw] 고장 주입: ch%d 토크 불응답(명령 무시·health ok)\n", f_stuck_ch);
    return true; }
  int read(HwState& s) override {
    if(on){
      for(int i=0;i<n;i++){
        if(i==f_stuck_ch && t>=fault_at){ dq[i]=0.f; continue; }   // ★불응답: 명령을 조용히 무시
...
struct JointMap {
  ...
  //   ★★2026-09-02 정정 — **근거가 틀렸었다. 동작은 그대로 둔다.**
  //   종전 근거: "Emb 가 ±180 을 래핑해서 181° 가 −179° 로 뒤집혀 반대편으로 날아간다."
  //   실제로는 그런 코드가 **없다**:
  //     · while(fPosition>180) 래핑 루프는 오래전부터 주석 처리돼 있었고,
  //       2026-09-02 벤더 업데이트에서 **소스에서 아예 삭제**됐다.
  //     · ±180 클램프(halGait.cpp:716-717)는 살아 있지만 m_fGaitTgt_Position 에만 걸린다
  //       = 벤더 자체 경로(PosCmd/TestCmd)와 기동 램프. **우리와 무관하다.**
  //     · 우리 명령은 MotCmd 모드로 들어가 halGait.cpp:782 의
  //         memcpy(&m_stSettingCmd[i], &m_stMotorCommand[i], sizeof(MotGeneral_t))
  //       가 구조체를 **통째로 덮으므로** 그 클램프를 지나가지도 않는다.
  //   ⇒ **Emb 안에 가드가 없다.** 위치·게인·토크가 그대로 MD80 까지 간다.
  //     이 포화가 체인 전체에서 **유일한 가드**다 — 그래서 더 지워선 안 된다.
  //   ⚠실패모드도 바뀐다: "반대편으로 튄다"(점프)가 아니라 **조용히 ±180 에서 잘린다**.
  //     240° 를 요구하면 180° 로 가서 머문다 — 경고 없이 60° 오차다. 점프보다 **안 보인다.**
  //     그래서 아래 n_saturated / last_saturated_ch 가 종전보다 더 중요하다.
  //   ⚠이름이 WRAP_DEG 지만 하는 일은 **포화**다. 이름에 속지 말 것(호출부가 있어 안 바꿨다).
