...
def main():
    ...
    # ── ★Emb → 이 앱 인계 (2026-08-07 · 전제 갱신 2026-08-26) ──────────────
    #   Emb 는 기동 시 **초기화 게이트**를 지난다: 상태수신 100틱 + 램프 4500틱 @1kHz ≈ 4.6초.
    #   그동안 m_ucIsGaitInitialized = 0 이라 **SHM 명령이 통째로 무시된다 — 소프트웨어로는
    ...
    #       서술이라 2026-08-26 에 고쳤다. 낡은 채로 두면 "부팅했는데 로봇이 안 선다" 를
    #       고장으로 오진하게 된다(정상이다).
    #     ★램프 블록을 지우면 안 된다: 같은 경로 **끝에서** m_ucIsGaitInitialized = 1 을
    #       세우므로, 지우면 SHM 명령이 **영원히 수용되지 않는다.** 초기화 시퀀스는 남기고
    #       목표만 없앤 것이다 — 원복하려면 :627 한 줄만 지우면 된다.
    ...
    #     ⚠여기 적혀 있던 "kp 100/50/50/20·kd 5" 의 출처는 :796-797 의 **주석 처리된 옛 줄**
    #       (//<--20, //<--1)이다 — 살아 있는 코드가 아니었다. 죽은 줄을 근거로 삼지 말 것.
    #   ⇒ 요컨대 이 앱이 뜨는 시점에 Emb 는 **부팅 당시의 자세를 위 게인으로 잡고 있다.**
    #
