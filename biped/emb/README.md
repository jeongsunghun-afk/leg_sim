...
```
Δq_ch   = s·Δq_joint            (s = sign·gear_k)
τ_ch    = kp_ch · Δq_ch
τ_joint = s·τ_ch                (일률보존: τ_joint·q̇_joint = τ_ch·q̇_ch)

⇒ τ_joint = kp_ch · s² · Δq_joint = **kp_ch · gear_k² · Δq_joint**   (부호는 제곱에서 상쇄)
```

**`kp_joint = kp_ch · gear_k²`** · `kd` 도 같다. 단위는 **둘 다 Nm/rad**.

| kind | `gear_k` | `k²` | `kp_ch`(배포) | **`kp_joint`** |
|---|---|---|---|---|
| hip | 1.0 | 1.00 | 100 | **100** |
| thigh | 1.0 | 1.00 | 50 | **50** |
| calf | **1.5** | **2.25** | 80 | **180** |
| foot | **1.2** | **1.44** | 30 | **43.2** |

⚠`hip`·`thigh` 는 `gear_k=1` 이라 같아서 **헷갈리지 않는다.** calf·foot 만 갈린다 —
그래서 실수해도 두 축에서만 티가 난다.

| 어디 | 어느 값 |
|---|---|
| `spec.yaml` · SHM · 드라이버 | **채널** `kp_ch` |
| MuJoCo 롤아웃 · WBIC · 데드밴드 계산 | **관절** `kp_joint` |
| `collect_multichirp` 가 저장하는 npz | **관절** (`kp_joint`·`kd_joint`) |
