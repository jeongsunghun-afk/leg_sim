...
측정 방법 — 토크 램프로 기동(breakaway)을 찾는다:
  Kp=Kd=0 으로 두고 tau_ff 를 **중력 바이어스**부터 아주 천천히 올린다.

  ★왜 0 이 아니라 바이어스인가 (2026-08-12)
    0 부터 올리면 **중력이 큰 축은 아예 못 잰다**:
      hip  중력 5.25 Nm(채널) — 기동 0.65 를 재려면 +dir 에 5.90 Nm 이 필요한데
           tau_max 가 1.4 라 도달 자체가 불가능하다. 자세를 바꿔도 안 준다
           (다리를 좌우로 여는 방향이라 thigh·calf 각과 무관).
      calf 중력 0.81 Nm > 기동 0.65 — **kp=kd=0 을 주는 순간 이미 미끄러진다.**
           τ_ff=0 인데 기동이 잡히니 측정이 성립하지 않고, 하위 링크가 떨어진다.
...
def probe_torque_mode(hw, spec, joint, log=print) -> dict:
    ...
    for direction in (+1.0, -1.0):
        for k in range(trials):
            ...
            if bias != 0.0:
                ...
                #   축이 흘러가는 내내 중력과 어긋난 채라 기동값이 오염된다.
                #   HL_hip 시행0 이 그 사례다 — 혼자 시작각이 3.2° 달랐고(−7.81 vs −11.00)
                #   그 흐름이 기동으로 오검출돼 f<0.084 라는 모순된 값이 나왔다.
                ...
                if abs(creep) > move_deg * 0.5:
                    ...
                    raise RuntimeError(
                        ...
                        f"    **바이어스가 틀렸다** — 이 상태로 재면 기동토크가 오염된다.\n"
            ...
            #     기동 0.172Nm(τ_s 실측 0.95 의 1/5) → 탐색범위 이탈 q=41.97 ∉ [−60, 40]
            #   thigh 는 기울기가 ~0.1Nm/° 라 축이 몇 도만 움직여도 FF 가 그만큼 틀어지고,
            #   그 오차가 곧 순토크라 가속이 붙는다. 기동이 비정상적으로 낮게 잡히는 것도
            ...
            while True:
                ...
                time.sleep(hw.dt)
            # ★기동 뒤에는 **놓지 말고 세운다** (2026-08-14). 종전엔 시험축을 무여자로
            ...
            #   실측: 기동 검출 즉시 break 했는데도 +1.4° → **+41.97°** 로 40° 를 갔고
            #   탐색범위 [−60, +40] 를 넘어 SafetyAbort. 두 번 연속 **같은 값**이 나왔다.
            ...
            log(f"    {'+' if direction > 0 else '−'}dir #{k}: "
                + (f"기동 {tau_at_move:.3f} Nm 에서 움직임" if moved
    ...
    out = {"supported": supported, "trials": results, "ch": ch, "name": name,
           # ★기어정보를 결과에 실어 보낸다 — 기동토크는 **채널토크**라 리포트에서
    ...
    #   "기동 직후 가속도가 얼마였나" 같은 걸 확인할 방법이 없었다.
    try:
        import os
    ...
    if supported:
        ...
        log(f"  ✅ 순수 토크모드 **지원됨** — 평균 기동토크 {tb:.3f} Nm")
