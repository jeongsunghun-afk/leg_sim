...
★먼저 갈라야 하는 것 — **엔코더가 커플링을 보는가?**
  (A) 엔코더가 foot **모터축**에 있다  → 무릎이 움직여도 foot 채널각은 안 변한다.
      ...
      이 경우 보정은 **명령 쪽에만** 필요하다(측정은 이미 모터축 값이라 맞다).
  (B) 엔코더가 커플링 **뒤**를 본다      → 무릎을 움직이면 foot 채널각도 같이 변한다.
      ⇒ 측정·명령 **양쪽** 보정이 필요하다.
  이 스크립트는 **채널각끼리** 회귀해서 (A)/(B) 를 가른다.
  ★회귀를 **채널각 공간**에서 하는 이유: 우리 변환(sign·k·offset·커플링)을 거치면 그
    가정이 결과에 섞여 든다. 채널각끼리 재야 하드웨어가 실제로 뭘 하는지가 나온다.
  ⚠⚠**서보가 물린 상태(jog/hold)에서는 이 측정이 무의미하다** — 발목 모터가 우리
    보상 명령을 추종하므로 자기 명령을 되재게 된다(2026-08-10 실제로 그 함정에
    빠져 부호를 반대로 판정했다). 반드시 **off(무여자)** 로 두고 손으로 돌릴 것.
...
#   ⚠기구 커플링은 **calf→foot 하나뿐**이다(사용자 확인 2026-08-12). foot→calf 도 없다.
#     thigh-calf 항목은 커플링이 있다고 보는 게 아니라 **없음을 확인**하는 용도다
#     (기울기 ≈ 0 이 나와야 정상).
#   ★왜 굳이 확인하나 — 늘어진 자세에서 calf/thigh = −1.94 라는 비율이 4표본 내내
#     일정해서 한때 "미신고 thigh→calf 커플링 coef +2" 를 의심했다. **틀렸다.**
#     원인은 닫힌사슬 좌굴이다: 발목이 고관절 바로 아래에 머문 채 무릎만 앞으로 꺾이면
#         Lt·sin(a1) + Lc·sin(a2) = 0  →  a2/a1 = −Lt/Lc
#     링크 길이 thigh 230.0 · calf 245.0 mm  →  예측 −0.9388, 실측 −0.9383 (**오차 0.1%**).
#     관절각으로 환산하면 calf = a2−a1 = −1.94·a1 이 그냥 따라 나온다.
#     ⇒ 비율이 일정하다고 커플링이라 단정하면 안 된다. 기하 구속이 같은 흔적을 남긴다.
