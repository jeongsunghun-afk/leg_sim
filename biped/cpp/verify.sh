...
#
# ★★2점 stand 는 **GT 경로로만** 검증한다(사용자 확정 2026-08-14). 근거가 있다 —
#   양발이 계속 붙어 있어 **앵커가 한 번도 안 바뀌므로** leg-odometry 가 드리프트하지 않는다.
#   60초 실측: 추정 base(−0.01, −0.00, 0.439) vs GT(−0.01, −0.00, 0.438) — **1mm 이내**.
#   실기 바이너리(biped_deploy)는 stand 도 추정기를 거치지만(biped_deploy.cpp:354),
#   그 경로가 GT 와 사실상 같으므로 **이 GT 줄이 실기를 대표한다.**
#   ⇒ 반대로 43cm 드리프트는 **stepping 전용 문제**다(스텝마다 앵커가 바뀐다). 아래 줄 참조.
