...
snap marched over the TRAP stone fields (stones_trapv3_L{0..9}.csv, the v2 IRREGULAR fields).
...
a stone; strip->strip excluded, strip->first-stone included):
  ...
  (c) % of on-stone footfalls landing on designated TRAP stones (trapv3_meta.json).

Output: naive_trapv3_L{0..9}.csv (per-footfall) + per-level table on stdout.
...
def load_meta():
    ...
    tm = json.load(open(os.path.join(D, "trapv3_meta.json")))
    return pm, tm
...
def load_stones(lvl):
    rows = []
    with open(os.path.join(D, f"stones_trapv3_L{lvl}.csv")) as f:
        for r in csv.DictReader(f):
...
def run_level(lvl, tm_lvl, adv, x_snap, anchors, write_csv=True, rng=None,
    ...
    if write_csv:
        ...
        with open(os.path.join(D, f"naive_trapv3_L{lvl}.csv"), "w", newline="") as f:
