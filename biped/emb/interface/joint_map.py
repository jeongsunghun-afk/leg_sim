...
class JointMap:
    ...
    # ── ±180 포화 가드 ────────────────────────────────────────────────────
    #   ★★2026-09-02 정정 — **근거가 틀렸었다. 동작(포화)은 그대로 둔다.**
    #   종전 근거: "Emb 가 ±180 으로 래핑하므로 232.8° 가 −127.2° 로 뒤집혀 튄다."
    #   그런 코드는 **없다**. 래핑 while 은 오래전부터 주석이었고 2026-09-02 벤더
    #   업데이트에서 소스에서 삭제됐다. ±180 클램프(halGait.cpp:716-717)는 살아 있으나
    #   **벤더 자체 경로에만** 걸리고, 우리 명령은 halGait.cpp:782 의 MotCmd memcpy 가
    #   구조체를 통째로 덮어써서 그 클램프를 **지나가지도 않는다.**
    #   ⇒ **Emb 에 가드가 없다.** 우리가 안 자르면 그대로 MD80 까지 간다 —
    #     이 포화가 유일한 가드다.
    #   ⚠실패모드: 점프가 아니라 **조용한 절단**이다. 240° 를 요구하면 180° 에 머문다
    #     (경고 없이 60° 오차). 그래서 아래 포화 카운트·보고가 더 중요해졌다.
    ...
    def q_joint_to_ch(self, q_joint_deg) -> np.ndarray:
        ...
        ★마지막에 채널각을 ±180 으로 **포화(saturate)** 시킨다.
          **Emb 에는 가드가 없다**(2026-09-02 정정 — 종전 "래핑한다" 는 거짓이었다).
          우리가 안 자르면 그대로 MD80 까지 간다. 넘으면 조용히 ±180 에 멈춘다.
