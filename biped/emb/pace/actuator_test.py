...
def _mech_limit_box() -> dict:
    """**기구 한계**(biped_emb.yaml joints.min_deg/max_deg, 모델각) → 채널각 탐색범위.
    ...
    ★spec 의 q_min/q_max 와 목적이 다르다:
      ...
      정렬에 spec 탐색범위를 쓰면 arm 이 곧바로 '위치 한계 이탈' 로 트립한다.
      ⇒ 정렬 구간은 기구 한계, 시험 구간은 spec 한계. 탐색범위를 **두 개** 쓴다.
    """
...
def _ch_limit_box(spec: dict, pin_home: bool = False) -> dict:
    """spec.joints 의 q_min/q_max(=**모델각** 여유폭) → **채널각** 한계탐색범위.
    ...
    ⚠커플링 때문에 foot 의 채널각은 calf **모델각**에도 의존한다 → 탐색범위의 상은 탐색범위가
      아니다. 여기서는 꼭짓점 2^8 전수의 상을 감싸는 최소탐색범위를 쓴다. 즉 **바깥쪽으로
      보수적**이다 — 정당한 명령을 잘못 자르지 않는 쪽. 폭주 방어는 q 탐색범위가 아니라
      토크·속도·추종오차·드리프트 트립이 맡는다.

    pin_home=True — 다른 축을 **HOME 에 고정**한 채 그 축만 범위를 훑는다.
      이게 **시험 중 실제 조건**이다(홀드축은 잡혀 있다). 커플링 여유가 안 붙어
      탐색범위가 훨씬 좁고, 그만큼 위치한계 트립이 실제로 일을 한다.
...
def main() -> int:
    ...
    for ch in chans:
        ...
        #   위치한계는 채널별 탐색범위로 이미 갈라놨는데 토크만 안 갈라져 있었다.
            ...
            try:
                ...
                if not a.no_home:
                    try:
                        ...
                        #   조용히 "도착" 했다. 이제는 탐색범위를 채널별로 넘기고, 그래도 밖이면
                        ...
                        #   HR_calf 가 −95.38 로 늘어져 탐색범위 −93.0 밖이라 arm 에서 죽었다.
                        #   ⚠보호는 유지된다: 현재 자리는 허용하되 더 바깥으로는 못 간다.
                        #     정렬의 목적 자체가 탐색범위 밖 자세를 안으로 데려오는 것이다.
                        ...
                        if not (_La.q_min <= _q_arm <= _La.q_max):
                            ...
                            _log(f"  ⚠정렬 탐색범위를 현재각까지 확장 {_q_arm:+.2f}° "
                        ...
                        if a.solo and ch in _gt:
                            ...
                            if _g_meas != _g_meas:      # nan — 탐색범위 여유 부족
                                _log(f"  ⚠중력 실측 생략 — {_q_now:+.2f}° 가 탐색범위 "
                            ...
                            if _npt > 1:
                                ...
                                #   (2026-08-14 수정). 종전엔 정렬탐색범위 전체를 훑었는데
                                ...
                                #   ⇒ 현재각 중심 ±span/2 로 자르고 탐색범위로 clip 한다.
                    ...
                    #   시험탐색범위 밖일 수 있다. 느슨한 기구한계를 유지하고 그 사실을 알린다.
