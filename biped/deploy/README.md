...
| **전진 + 지연**(30s) | — | sense≤10ms·act≤5ms=**0** / sense20·act10ms=붕괴 (`SENSE/ACT_LAT_MS`) |

**배포 스펙**: 노이즈 강건(4배 OK), 그러나 **지연 마진 빠듯**(구동≤5ms·센서≤10ms). 점발 marginal이라 지연 민감.
→ 실HW **제어루프 고속(≤5ms 구동지연) 필수**, 초과 시 **지연보상**(모델 예측 전진) 필요. D-로드맵 잔여 관문.
