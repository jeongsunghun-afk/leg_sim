...
### A. 지금 바로 — foot τ_ff 파단시험  ★다음 작업

**왜 이게 먼저인가**: 위치처프는 **순환**이다. 드라이버가 돌려주는 τ 가 `kp·err + kd·derr`
로 R² 0.97 재구성된다(지연 10ms 정렬 시). 그 τ 로 회귀하면 우리 게인의 그림자를
식별하는 셈이다. `kp=kd=0` 이면 재구성할 항이 없고 **우리가 넣은 τ_ff 가 곧 입력**이다.
foot 부터인 이유는 중력이 −0.03 Nm 로 사실상 0 이라 흘러내림 위험이 가장 낮아서다.

```
① 지그로 홀드축 고정 (시험축 HL_foot 은 풀어 둘 것)
...
```
시퀀스: `arm → HOME 복귀 → 지그 대기 → 한계 조임 → 드라이버 생존확인 → 토크램프`

- [ ] **파단이 일어나는가** — 안 나면 `fTorque` 가 무시된다는 뜻이고, τ_ff 식별 계획
      전체가 무너진다. 그 경우 위치처프 + 전류 교차검증으로 우회해야 한다
- [ ] 파단토크 vs 위치모드 정지마찰(hip 0.63~0.71 Nm) — **교차검증**
- [ ] `check_hold` 통과 (= 지그 슬립 없음)
- [ ] 뷰어에 자세가 계속 뜨는지 (상태발행 실기 첫 검증)
- [ ] 성공하면 ch7(HR_foot) 반복 → 좌우 대칭성
...
### D. 미해결 — 영점 (신규, 2026-08-11 실측)

지그 자세 = HOME = 영점이어야 하는데 **thigh 두 축이 튄다**:
```
HL_hip +0.06  HL_thigh +4.78  HL_calf +0.71  HL_foot −1.80
HR_hip +0.50  HR_thigh +2.03  HR_calf −0.54  HR_foot −0.97   (모델각, 0 이어야 함)
```
- [ ] HL_thigh **+4.78°** 원인 규명 — ① 영점(offset) ② 지그가 HOME 자세 아님 ③ 기구 유격
- [ ] HR_thigh 는 교정 재현성이 이미 1.69° 로 최악이었다(나머지 ≤0.53°). 연관 확인
- [ ] 원인 확정 전까지 thigh 측정값은 **자세 불확실**로 표시할 것
