...
def _sweeps(hw, ch, cfg, kp, kd, q_center, log, ff=None) -> dict[float, tuple[float, float, float, float]]:
    ...
    for v in _use:
        ...
        for d in (+1.0, -1.0):
            ...
            # ★평균 창은 **반드시 통과 중앙에 대칭**이어야 한다 (2026-08-12 실기).
            #   종전엔 앞쪽만 max(lo·T, t_settle) 로 잘랐다. 고속에서 lo·T < t_settle 이
            #   되는 순간 **앞만 잘리고 뒤는 그대로**라 창이 뒤로 밀린다:
            #     thigh 80dps → T=0.375, lo·T=0.075 < 0.15 → 창 (0.150, 0.300)
            #     +dir 은 중앙보다 +3.0°, −dir 은 −3.0° 를 평균한다 = **6° 어긋남**
            #   f=(τ⁺−τ⁻)/2 는 "같은 위치를 양방향으로" 가 전제다. 어긋나면 그 구간의
            #   **중력차가 그대로 가짜 마찰**이 된다: thigh 기울기 0.11Nm/° × 6° ÷ 2
            #   = 0.33 Nm. 실측이 40dps 0.369 → 80dps **1.092** 로 튀었고, 직선적합이
            #   JFRIC **−0.455**(음수, 물리적으로 불가능)를 냈다.
            #   ⚠calf 는 멀쩡했다 — 중력 기울기가 0.004 Nm/° 로 thigh 의 1/27 이라
