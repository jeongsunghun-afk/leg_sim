...
### A-2. 다음 작업 — foot 관성 측정 (`--tests inertia`)

```
① 지그로 홀드축 고정 (HL_foot 은 풀어 둘 것)
② python3 actuator_test.py --ch 3 --tests inertia --jig
```
- [ ] `I_joint` 를 MJCF 예측 **0.0542** 와 대조 — 이게 τ_ff **스케일**의 간접 검증도 된다
- [ ] 절편의 쿨롱마찰이 파단토크 0.674 Nm(채널)과 같은 크기인지 확인 (교차검증)
- [ ] 방향간 I 편차 10% 이내인지
- [ ] 성공하면 calf(ch2) → thigh(ch1) → hip(ch0) 순으로 확대

⚠준위 [0.90, 1.10, 1.30] Nm 는 파단 0.674 위로 잡았다. 미만이면 안 움직인다.
⚠순수토크는 자기제한이 없다 — 12° 이동 또는 120 dps 도달 즉시 토크 0 + 위치게인 제동.
