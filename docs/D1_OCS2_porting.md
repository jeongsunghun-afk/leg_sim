...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **★★종합 결론(전문가 시퀀스 전수 실행)**: #1 seed(무죄)·#2 relaxed barrier(실패)·#3 DDP(실패)·SQP/R/g_max/coldStart/nThreads/가중(모두 robust 실패). **동적 model-based locomotion=전환의 MPC solver 견고성+WBC contact-mode 처리의 통합 난제**로, 단일 개입으로 안불림. **메모리 확정방향([[biped-wbic-mpc-project]] 험지·동적보행=RL) 재확인** — model-based는 정적 solid까지, 동적/험지는 RL(DTC)가 실질경로. 남은 model-based 옵션(고비용): #4 SqpSolver box trust-region+WBC 실제접촉감지, or crocoddyl 전신 CI-MPC. **#5 출력 feasibility gate**=배포 안전판(별개). **WBC 정적·MPC standalone 정확검증**. env노브=W_BASE/W_F/W_SW·KP_*·SWING_JOINT·SETTLE·DDP·**SOFT_ZF**·MPC_HZ·SQP·g_max·nThreads·TROT_DBG.
