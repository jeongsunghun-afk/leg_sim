...
class Hardware:
    ...
    def read(self, ch: int) -> tuple[float, float, float, float]:
        ...
        #   4차: 그래도 **기동푸시에서 오탐했다** — 실기 ch2 "상태 정지 161ms".
        #        기동푸시는 명령을 0.6dps 로 기어가게 하므로 바이트는 매 틱 바뀐다
        ...
        #       기동푸시 0.6dps → 0.5° 가는 데 833ms (오탐 없음)
    ...
    def _check_impl(self, ch: int, q: float, dq: float, tau: float, q_cmd: float) -> None:
        ...
        #   ⚠**의도적 정지압박**(토크램프·기동푸시)에서는 꺼야 한다 — intentional_push() 참조.
        if self.stall_watch and self.grav_fn is not None:
            ...
            if abs(tau) - g <= self.stall_margin_nm:
                ...
                if moved >= self.stall_move_deg:
                    ...
                    #   탐색범위 끝과의 거리를 함께 주면 첫 줄만 읽고도 판별된다.
                    ...
                    raise SafetyAbort(
                        ...
                        f"  위치 {q:+.2f}° · 명령 {q_cmd:+.2f}° · 탐색범위 [{L.q_min:+.1f}, "
                        f"{L.q_max:+.1f}]° — 가까운 탐색범위 끝까지 {_d:+.2f}°.\n"
                        f"  {'탐색범위 끝이다 → **기구 스톱**을 밀고 있다.' if _d < 3.0 else '탐색범위 한가운데다 → 기구스톱이 아니라 **간섭·중력모델오차·케이블**이다.'}\n"
    ...
    def brake(self, ch: int, kp: float, kd: float, hold_s: float = 0.3,
        ...
        ★2026-08-12 실기에서 limp 가 두 축을 탐색범위 밖으로 날렸다:
    ...
    def measure_gravity(self, ch: int, kp: float, kd: float, ff_fn=None,
        ...
        ⚠기동(breakaway) 전에 불러야 의미가 있다. 홈복귀부터 보호가 필요하기 때문이다.
        """
        # ★스톨 감지를 끈다 — 여기서 "안 움직이는데 토크가 크다" 는 **측정 대상 자체**다.
        #   게다가 그 감지가 쓰는 중력이 바로 지금 고치려는 그 틀린 값이다(순환).
        #   ⚠상한은 여전히 τ_trip 과 위치탐색범위가 쥔다. 움직임도 ±delta_deg 로 묶여 있다.
        # ★탐색범위 안에 **움직일 자리가 있는지** 먼저 본다 (2026-08-12 실기 HR_hip).
        #   오른다리를 놓으면 hip 이 바깥으로 벌어져 한계(±14.9°)에서 1.1° 앞에 선다.
        #   거기서 ±4° 를 움직이면 **기구 스톱을 밀며** 측정하게 되고, 그 토크는 중력이
        #   아니다. 실제로 HR_hip 중력을 2.600 으로 쟀는데(표 3.812) 그 뒤 홈복귀가
        #   FF 부족으로 스톨했다.
        #   ⇒ 양쪽에 1° 여유를 두고 delta 를 줄인다. 그마저 없으면 **재지 않는다**
        #     (nan 을 돌려주고 호출부가 표를 그대로 쓰게 한다). 틀린 측정보다 낫다.
        q0 = float(self.read(ch)[0])
        L = self.limits_for(ch)
        room = min(q0 - L.q_min, L.q_max - q0) - 1.0
        if room < 1.0:
            return float("nan")
        delta_deg = min(delta_deg, room)
        got = []
    ...
    def verify_driver_live(self, ch: int, kp: float = 40.0, kd: float = 2.0,
        ...
        if spread < move_floor_deg:
            raise SafetyAbort(
                ...
                f"    **복구에 성공하고도 실패로 읽는다.**\n"
                f"    (게이트 자체는 그대로다: 상태수신 100틱 + 램프 4500틱 @1kHz ≈ 4.6초.\n"
                f"     그동안 SHM 명령이 통째로 무시되므로 재기동 후 5초는 기다렸다 붙을 것.)\n"
                f"  ⇒ 지금의 성공 판정은 **이 검사를 다시 돌리는 것**이다: 5초를 기다린 뒤\n"
    ...
    def run_torque(self, ch: int, tau_fn, duration_s: float, tau_max: float,
        ...
        **우리가 넣은 τ_ff 가 곧 입력**이라 순환이 원천 소멸한다.
        ...
        #   (기동 직전이 정확히 그 상태다). 상한은 step_torque 의 tau_max 가 쥔다.
        try:
            with self.intentional_push():
                while True:
                    t = time.monotonic() - t0
    ...
    def _raw_write_all(self, q_cmd_vec, kp, kd, q_box=None) -> None:
        ...
        ★축별 dict 를 받는 이유: 스칼라 하나로는 못 맞춘다. spec 의 kp 40 은 다리 미장착
          ...
          배포게인(hip100/thigh50/**calf80**/foot30 · kd 6/4/3.5/2)을 그대로 쓰는 게 맞다.
