...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **★★종합 결론(최종·확정)**: legged WBC 이식 실패는 **솔버도 코드버그도 로봇/물리도 아님**. legged WBC는 **연속 고속 MPC 폐루프 + HW kd댐핑과 co-design된 내부루프**라(그 루프가 로봇을 nominal 유지→순수FF 자기교정·unconditional consume 안전), 루프 없이 WBC만 떼면 (a)FF드리프트+(b)무댐핑+(c)QP추격실패로 발산. **수정=PD 되먹임+관절댐핑=사실상 우리 custom WBC·A WBIC와 동일 원리로 수렴, 그리고 실제로 STANCE를 세움**. 즉 "다른 데선 되는데 우리는 안 됨"의 답=우리 로봇/물리/솔버는 정상이고(우린 이미 동작 WBC 보유), legged 특정 WBC를 그 루프 없이 이식한 게 문제. 완전 legged식 이식엔 연속 스레드MPC 필요, 아니면 PD로 바꾸면 됨(=custom WBC). 동적보행 산출물은 RL 트랙(타처)이 커버. 자산=faithful포트·qpOASES·계측토글(LEGDBG/BASE_PD/BASE_NOFF/JKD/NO_FALLBACK)·**작동 STANCE 설정(BASE_NOFF+BASE_PD+JKD3)**. **정직한 다음 옵션**: (a)**브리지를 스레드 연속 MPC로 재구성**(legged 아키텍처, MPC 별도스레드+WBC가 최신정책 스왑) → FF가 제대로 작동, (b)legged_control 전체 스택 이식(ROS1/Gazebo), (c)메모리 확정방향=RL(DTC) 피벗. **정적 STANCE solid·MPC standalone 계획 정확은 불변**. 실험토글=FF_BASE·SOFT_ZF·DDP·SWING_JOINT·SETTLE 전부 기본 off면 정적 solid.
