...
def _sweeps(hw, ch, cfg, kp, kd, q_center, log, ff=None) -> dict[float, tuple[float, float, float, float]]:
    ...
    for v in _use:
        ...
        for d in (+1.0, -1.0):
            ...
            # ★평균 창을 **시간이 아니라 위치**로 잡는다 (2026-08-12, 3차이자 마지막).
            #   f=(τ⁺−τ⁻)/2 는 "**같은 절대위치**를 양방향으로" 가 전제다. 시간 창은
            #   그 전제를 못 지킨다 — 축이 명령을 지연해서 따라가기 때문이다:
            #     등속 추종오차 = (마찰 + kd·v)/kp  →  속도에 따라 커진다
            #     +dir 은 명령보다 뒤, −dir 은 앞 → 두 통과의 실제 위치가 2×오차만큼 다르다
            #   thigh 중력 기울기 0.11 Nm/° 가 그 차이를 그대로 **가짜 마찰**로 바꿨다:
            #     예측 −0.093/−0.164/−0.252/−0.428/−0.604 Nm (2/10/20/40/60dps)
            #     실측 −0.041/−0.055/−0.169/−0.358/−0.642 → 자릿수·부호 일치.
            #     그래서 마찰이 속도에 따라 줄다 **음수(−0.082)** 까지 갔다.
            #   ⚠calf 는 기울기가 1/27 이라 이게 안 보였다. thigh 에서만 드러나는,
            #     오늘 네 번째 "대칭 가정이 깨지는" 부류다.
            #   ⇒ 중심 q_center 에서 ±w 인 **위치 띠**를 쓴다. 지연이 얼마든 두 통과가
            #     같은 절대위치를 평균하므로 중력이 정확히 상쇄된다.
            #   ⚠띠는 과도구간 **밖**이어야 한다. 정착 t_lo 동안 지나간 거리만큼
            #     양쪽에서 **대칭으로** 줄인다(한쪽만 줄이면 다시 비대칭이다).
            t_lo = max((1 - frac) / 2 * T, _t3)
            w = min(frac * half, half - v * t_lo)
            if w <= 0.5:
                log(f"    ⚠ {swing_str(cfg['stroke_deg'], v)}: 정착({t_lo:.3f}s)에 "
                    f"{v * t_lo:.1f}° 를 써서 남는 띠가 {2 * w:.1f}° 뿐 — 이 점 제외")
                res = {}
                break
            m = np.abs(a["q"] - q_center) <= w
        ...
        if len(res) == 2:
            ...
            # ★짝을 **위치로** 맞춘다 — 시간으로 맞추면 안 된다 (2026-08-12).
            #   전체 창은 통과 중앙에 대칭이지만 **반으로 가르면** 이렇게 된다:
            #     +dir 앞반쪽 = 절대위치 [c−h, c]      −dir 앞반쪽 = [c+h, c]
            #   즉 (+앞 vs −앞) 은 **정반대 위치**라 중력이 안 지워진다.
            #   thigh(기울기 0.11Nm/°)에서 그 가짜 차이가 1.32Nm 로 나와, 2dps 부터
            #   **여섯 속도 전부**가 "등속 미도달" 로 잘못 버려졌다(실측 차이 1.156 —
            #   예측과 일치). calf 는 기울기가 1/27 이라 0.048Nm 뿐이어서 통과했다.
            #   ⇒ 같은 위치끼리 짝짓는다: (+앞 ↔ −뒤) · (+뒤 ↔ −앞).
            #     그러면 중력이 지워지고 **가속만 남는다** — 검사의 원래 목적 그대로다
            #     (가속은 +pass 앞쪽에만 실리므로 두 짝이 I·α 만큼 갈린다).
            # ★두 통과가 **같은 자리를 평균했는지** 먼저 본다 (2026-08-12 실기 HL_calf).
            #   f=(τ⁺−τ⁻)/2 의 전제가 그것이다. 실기 20dps 에서 평균위치가
            #   **+7.53 / +0.03** 로 7.5° 떨어졌는데(속도도 20 명령에 23.1 실측),
            #   "속도 산포 과대" 경고만 뜨고 **그 점이 회귀에 들어갔다**.
            #   중력 기울기가 작은 calf 라 티가 덜 났을 뿐 전제 위반이다.
            #   ⇒ 띠 폭의 25% 를 넘게 어긋나면 버린다. 정상은 0.2° 이내다.
            _dp = abs(res[+1.0][6] - res[-1.0][6])
            if _dp > 0.25 * res[+1.0][5]:
                log(f"    ⚠ {swing_str(cfg['stroke_deg'], v)} ({v:.0f}dps): 두 통과의 "
                    f"평균위치가 {res[+1.0][6]:+.2f} / {res[-1.0][6]:+.2f} 로 "
                    f"{_dp:.2f}° 어긋났다(띠 ±{res[+1.0][5]:.1f}° 의 25% 초과). "
                    f"**±상쇄 전제 위반 — 이 점 제외**")
                continue
...
def measure_actuator_friction(hw, spec, joint, plotdir, log=print) -> str:
    ...
    if jfric < 0:
        # ★터미널에도 **크게** 찍는다 (2026-08-12). 종전엔 report 의 warn 에만 들어가
        #   터미널에는 "JFRIC=-0.4547" 이 아무 표시 없이 지나갔다. 실기에서 그 줄만
        #   보고 값을 받아적을 뻔했다. 물리적으로 불가능한 값은 눈에 띄어야 한다.
        msg = (f"쿨롱마찰이 **음수**({jfric:.4f}) — 물리적으로 불가능하다. "
               f"±상쇄 전제가 깨졌다는 뜻이다.\n"
