...
#   ⇒ **왼쪽 무릎 벨트/풀리 미끄러짐.**
#
# ⚠⚠HL_thigh — **측정값 1.50 을 운용에 쓰지 않는다. 운용값은 HR 미러 1.10 이다.**
#   1.50 은 측정 시점의 미끄러짐을 흡수한 값인데, 그대로 적용하자 실기에서 **HL 이
#   과잉으로 다리를 밀어올렸다**(2026-08-24 저녁 실사용). 측정과 운용 사이에 벨트가
#   또 움직였다는 뜻이다 — **벨트가 유동 중이라는 추가 증거**이고, 그래서 이 축의
#   어떤 고정 배율도 신뢰할 수 없다. 1.10 으로 낮추자 "매우 잘 된다"(실사용 확인).
#   **walk 는 왼무릎을 수리하기 전엔 금지.**
#   수리 후: 지그 영점 재캘리브레이션 → gen_grav_table --apply → g* 재측정(좌우가
#   같은 값으로 재현되면 수리 성공) → 이 파일 갱신.
#
...
export GRAV_SCALE_JOINT="1.20,1.10,1.00,1.00,1.18,1.10,1.00,1.00"
...
export STAND_TAU_SCALE_JOINT="1.20,1.10,1.00,1.00,1.18,1.10,1.00,1.00"
