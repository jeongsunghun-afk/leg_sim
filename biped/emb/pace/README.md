...
## 세 가지 시험 — **무엇을 흔들고 무엇을 얻나**

이름이 비슷해 헷갈린다. 갈리는 지점은 **무엇을 명령하느냐**다.

| | `--tests torque` | `--tests friction` | **PACE 다축 처프** |
|---|---|---|---|
| 명령하는 것 | **토크**(`kp=kd=0`) | **위치**(등속 왕복) | **위치**(전 축 동시 처프) |
| 각도의 역할 | `travel` — **막아야 할 것** | 고속을 내기 위한 수단 | **가진 그 자체** |
| 흔드는 양 | 토크 준위 6개(자동설계) | 스트로크 24~56° (±12~28°) | 진폭 **±5~20°** |
| 속도 | `vref` 100 dps 에서 읽음 | 2~120 dps 단계별 | 최대 148 dps |
| **얻는 것** | `I_total` · **기동토크** τ_s | `f(q̇) = τ_c + b·q̇` → **절편·기울기** | `ROTOR_I`·`JDAMP`·`JFRIC`·`bias`·`delay` |
| 실행 | `actuator_test.py --tests torque` | `--tests friction` | `collect_multichirp.py` → `pace_cmaes.py` |

★**왜 셋 다 필요한가** — 다축 처프만으로는 **원리적으로 못 가르는 것이 셋** 있다
(`RESULTS.md` §0-b). 각축 시험이 그걸 바깥에서 못박아 초기값·탐색범위를 준다.

★**언제 어느 것을 믿나**

| 알고 싶은 것 | 쓸 시험 | 왜 |
|---|---|---|
| 링크 관성 `I_link` | `torque` | 토크를 알고 `q̈` 를 재면 `I` 가 바로 나온다. **`t_ref→0` 외삽**이 관성 편향을 뺀다 |
| 점성 `b` | `friction` | `q̈ = 0` 이라 관성이 사라진다 — **기울기가 곧 `b`** 다 |
| 쿨롱 `τ_c` | 둘 다 + PACE | 스윕은 과대평가(유한속도), 토크절편이 정의에 맞다. `RESULTS.md` §0 ◆ |
| 커플링·지연·영점 | PACE | 전 축이 같이 움직여야 살아나는 항이다 |

⚠**각축 시험에서는 커플링 항이 죽는다** — 타축을 잡으므로 `q̇_calf = 0` 이고
raw각 = 관절각이다. `foot` 의 반사관성·마찰·감쇠를 모터축에 둬야 한다는 것이
여태 안 보였던 이유가 이것이다(`RESULTS.md` §1-b).
...
## SHM 에 실제로 무엇이 나가나 — 쓰기 경로 셋

`hwio` 는 세 가지 방식으로 SHM 에 쓴다. **어느 필드가 채워지는지가 다르다.**

| | `step()` — 위치제어 | `step_torque()` — 순수토크 | `limp()` |
|---|---|---|---|
| `fPosition` | `q_cmd` | `q_des` 있으면 그 값, 없으면 **현재 측정각** | 현재 `_q_cmd` |
| `fVelocity` | 0 | 0 | 0 |
| `fTorque` | `tau_ff`(있을 때) | **`tau_cmd`** (`tau_max` 로 클립) | 0 |
| `fGainKp` | 시험축 `kp` · 홀드축 각자 값 | **`kp` 인자**(기본 0) · 홀드축 각자 값 | **전 채널 0** |
| `fGainKd` | 시험축 `kd` · 홀드축 각자 값 | **`kd` 인자**(기본 0) · 홀드축 각자 값 | **전 채널 0** |
| `bridge_enable` | 1 | 1 | **0** |

⚠**홀드축은 순수토크 중에도 계속 잡혀 있다.** `_hold_gains(ch)` 가 zeros 로 시작해
`hc != ch` 인 축만 채우고, 그다음 **시험축 자리를 `kp`/`kd` 로 덮어쓴다.**
여기서 전 채널을 0 으로 두면 다리 전체가 무여자가 되어 중력으로 접힌다
(다리 조립 후 특히 위험).

### ★`step_torque` 의 `kp`/`kd`/`q_des` — **게인→토크 핸드오프** (2026-08-12)

"순수 토크" 인데 왜 게인 인자가 있나:

> 중력이 큰 축(hip 5.25 · calf 0.81 Nm)은 게인을 **먼저** 0 으로 놓으면 바이어스를
> 올리기도 전에 떨어진다 — 드라이런에서 hip 이 **207 dps** 로 트립했다.

⇒ `kp`·`kd` 를 홀드값에서 0 으로 **내리면서** `tau_ff` 를 0→bias 로 **올린다.**
⚠그동안 `q_des` 를 **출발점에 고정**해야 한다. 기본값처럼 `q_cmd = q`(현재각)로 두면
오차가 0 이라 `kp·err` 가 잡는 힘을 못 낸다.

⚠`fPosition` 은 순수토크(`kp=0`)에서 **의미가 없다** — `limp` 복귀용으로 현재값을
유지할 뿐이다. 추종오차 검사도 `q_cmd = q` 로 넣어 무력화한다(`_check`).
...
| | 축별 해석회귀 | **PACE 전축 동시** |
|---|---|---|
| 가진 | 한 축만 τ_ff · **나머지 7축 무여자**(kp=kd=0, 손으로 잡는다) | **전 축 동시**(비상관 처프) |
