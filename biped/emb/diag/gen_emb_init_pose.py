...
════════════════════════════════════════════════════════════════════════════════
  ...
  들어가고 되읽기 검증도 통과한다. 그런데 **그 배열이 매 부팅 통째로 덮어써진다**:
      ...
      halGait.cpp:627   ← 2026-08-10 '기동 램프 = 제자리 유지' 패치
  ...
  ★다시 유효해지는 조건: halGait.cpp:627 **그 한 줄만 지우고** 다시 빌드하면 된다.
...
문제(:627 원복 시):
  ...
  으로 보낸다(halGait.cpp:641-687). 벤더 기본값은 전부 0 인데 그 0 은 **채널각 0** 이고,
...
좌표계가 같다는 근거 (이게 성립해야 그대로 넣을 수 있다):
  · m_fGaitCfg_Dir 8축 전부 1.0            (halGait.cpp:152-159)
...
⚠--patch 만으로는 로봇 거동이 바뀌지 않는다. 맨 위에 적었듯 halGait.cpp:627 을 함께
...
def main() -> int:
    ...
    print("     halGait.cpp:627 이 매 부팅 m_fGaitCmd_PositionInit[] 을 측정각으로 덮어쓴다")
    ...
    print(f"\n  ±180 초과: {over if over else '없음 ✅'}"
          f"    (Emb 는 fDir·target 을 ±180 으로 **포화**시킨다 — halGait.cpp:780-781)")
    if over:
    ...
    #   그래서 조건부로 갈라 적는다.
    print(f"  ⚠ 이 값이 실제로 쓰이는지는 halGait.cpp:627 을 지웠는지에 달려 있다:")
