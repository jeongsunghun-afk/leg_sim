...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **잔여(확정, code-level)**: **#2 relaxed barrier ramp**(전환 인접 1~2노드 swing zero-force·마찰콘을 hard g→soft cost=RelaxedBarrierPenalty, constraint-reduction 모드 미발동) — DDP도 실패라 이게 가장 유망. or **#4 SQP box trust-region 미니패치**(HPIPM box \|δu\|≤Δ). **#5 출력 feasibility gate**(fDes≤k·mg·rate limit·hold-last, 안전판). **WBC 정적·MPC standalone 정확검증**. env노브=W_BASE/W_F/W_SW·KP_*·SWING_JOINT·SETTLE·**DDP**·MPC_HZ·SQP·g_max·nThreads(task.info)·TROT_DBG.
