...
3. **Phase 2b — QP WBC 신규 구현** 🔶 **진행중(2026-07-29, 사용자 선택=QP WBC)**: `test/wbc_02leg.hpp` = weighted QP WBC(legged_control식). 결정변수 [q̈(18), f(12)]. hard=floating-base 동역학(6)·stance no-slip(3/발)·swing force0·friction cone. soft=base 6D 가속·swing 발 가속·f_des 추종. τ=[Mq̈+h−Jcᵀf]_actuated. eiquadprog(conda, dynamic-double ABI안전).
   ...
   - **잔여(확정된 다음 작업, solver 엔지니어링)**: **OCS2 `SqpSolver`에 trust-region/step-limiting 추가**(필터가 cost 폭발 스텝을 거부하도록, 또는 max step-norm 제한). = third-party solver 코드 수정=실질 fix. config 튜닝 범위 밖. 대안: (i)crocoddyl 등 trust-region 있는 solver, (ii)DDP(SLQ)로 전환. **WBC 정적·MPC standalone은 정확 검증됨**. env노브=W_BASE/W_F/W_SW·KP_*·SWING_JOINT/KP_JS·SETTLE·MPC_HZ·SQP·g_max(task.info)·TROT_DBG.
