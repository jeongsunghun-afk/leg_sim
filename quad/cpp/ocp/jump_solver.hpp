#pragma once
// S3-b Step2 — 점프 OCP를 호출가능 함수로 추출. jump_ocp.cpp(파일생성)와 trot_view(루프내 live-solve) 공용.
//   crocoddyl FDDP 다상: push(4접촉,vz↑) → flight(무접촉) → land(4접촉). 허리 lock=16 leg DOF.
//   결과 궤적을 MuJoCo 17-DOF(qpos순, 허리=0)로 변환해 반환(= 배포 replay 포맷과 동일 인메모리).
#include <string>
#include <vector>
#include <cmath>
#include <iostream>
...
// OCP를 풀어 점프 궤적을 반환. urdf/mjcf 로드 + crouch IK + FDDP solve + pin→mj 변환.
inline JumpTraj jump_solve(const std::string& URDF, const std::string& MJCF,
  ...
  // ── q_crouch·q_stand: MuJoCo crouch IK(mj_crouch) → pinocchio 매핑(mj2pin) ──
  char merr[1000];
  mjModel* mjm = mj_loadXML(MJCF.c_str(), nullptr, merr, 1000);
  if (!mjm) { std::cerr << "[jump_solve] mjcf 로드실패: " << merr << "\n"; return R; }
  ...
  std::vector<VectorXd> us(models.size(), VectorXd::Zero(nu));
